...
static void NamingScreen_CreateWaldaDadIcon(void)
{
    ...
    spriteId = CreateObjectGraphicsSprite(OBJ_EVENT_GFX_SUNRISE_MAN_1, SpriteCallbackDummy, 56, 37, 0);
    gSprites[spriteId].oam.priority = 3;
    StartSpriteAnim(&gSprites[spriteId], ANIM_STD_GO_SOUTH);
}
...
static void NamingScreen_CreateRivalIcon(void)
{
    ...
    rivalGfxId = GetRivalAvatarGraphicsIdByStateIdAndGender(PLAYER_AVATAR_STATE_NORMAL, sNamingScreen->monSpecies ^ 1);
    spriteId = CreateObjectGraphicsSprite(rivalGfxId, SpriteCallbackDummy, 56, 37, 0);
    gSprites[spriteId].oam.priority = 3;
    StartSpriteAnim(&gSprites[spriteId], ANIM_STD_GO_SOUTH);
}
...
static void DrawMonTextEntryBox(void)
{
    ...
    u8 *end = StringCopy(buffer, GetSpeciesName(sNamingScreen->monSpecies, SKIP_NAME_CHECK));
...
void NameRival(void)
{
    if (gSaveBlock2Ptr->playerGender == MALE)
        StringCopy(gSaveBlock2Ptr->rivalName, gFemalePresetNames[0]);
    else
        StringCopy(gSaveBlock2Ptr->rivalName, gMalePresetNames[0]);
