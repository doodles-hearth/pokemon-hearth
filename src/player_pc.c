...
{
    StringCopy(gStringVar1, gSaveBlock1Ptr->mail[0].playerName);
...
static void Mailbox_FadeAndReadMail(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        MailboxMenu_Free();
        CleanupOverworldWindowsAndTilemaps();
        ReadMail(&gSaveBlock1Ptr->mail[0], Mailbox_ReturnToFieldFromReadMail, TRUE);
...
{
    struct Mail *mail = &gSaveBlock1Ptr->mail[0];
...
static void ItemStorage_PrintMenuItem(u8 windowId, u32 id, u8 yOffset, u8 itemPos)
{
    if (id != LIST_CANCEL)
    {
        if (sItemStorageMenu->toSwapPos != NOT_SWAPPING)
