...
static uq4_12_t GetTypeEffectivenessForSmokeExplosion(enum BattlerId battler)
{
...
s32 CalcSmokeExplosionDamage(enum BattlerId battler)
{
...
static uq4_12_t GetWeatherDamageModifier(struct DamageContext *ctx)
{
    ...
    if (ctx->weather & B_WEATHER_LEAVES || attackerWeather & B_WEATHER_LEAVES) {
        switch (ctx->moveType) {
            case TYPE_GRASS:
            case TYPE_FAIRY:
                return UQ_4_12(1.5);
