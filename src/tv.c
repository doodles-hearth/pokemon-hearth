...
static void InterviewBefore_FanClubLetter(void)
{
    ...
    {
        StringCopy(gStringVar1, GetSpeciesName(GetMonData(&gPlayerParty[GetLeadMonIndex()], MON_DATA_SPECIES, NULL), SKIP_NAME_CHECK));
...
static bool8 IsPartyMonNicknamedOrNotEnglish(u8 monIdx)
{
    ...
    if (language == GAME_LANGUAGE && !StringCompare(GetSpeciesName(GetMonData(pokemon, MON_DATA_SPECIES, NULL), SKIP_NAME_CHECK), gStringVar1))
        return FALSE;
