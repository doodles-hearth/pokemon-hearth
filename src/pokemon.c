...
static const enum NationalDexOrder sHoennToNationalOrder[HOENN_DEX_COUNT - 1] =
{
    ...
    HOENN_TO_NATIONAL(PSYDUCK_TOKUAN),
    HOENN_TO_NATIONAL(GOLDUCK_TOKUAN),
    ...
    // HOENN_TO_NATIONAL(FLUFFROST),
    // HOENN_TO_NATIONAL(YETIZZARD),
...
#if P_LVL_UP_LEARNSETS >= GEN_HEARTH
#include "data/pokemon/level_up_learnsets/hearth.h" // Gen 6 with tweaks
...
const u8 *GetSpeciesName(enum Species species, enum SpeciesNameCheck nameCheck)
{
...
const u8 *GetSpeciesPokedexDescription(enum Species species, enum SpeciesNameCheck check)
{
    species = SanitizeSpeciesId(species);
...
void AdjustFriendship(struct Pokemon *mon, u8 event)
{
    ...
    if (species && species != SPECIES_EGG)
    {
        ...
        if (event == FRIENDSHIP_EVENT_LEAGUE_BATTLE)
        {
            ...
            if (!(opponentTrainerClass == TRAINER_CLASS_DOJO_MASTER
