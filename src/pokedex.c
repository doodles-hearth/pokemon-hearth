    ...
    case ORDER_NUMERICAL:
        for (i = 0, r5 = 0, r10 = 0; i < temp_dexCount; i++)
        {
            ...
            if (r10 || temp_isHoennDex)
            {
                ...
                if (IsMonSeenNamedOrDescribed(sPokedexView->pokedexList[r5]))
...
u8 CreateMonName(enum NationalDexOrder num, u8 left, u8 top)
{
    ...
    if (species)
        str = GetSpeciesName(species, DO_NAME_CHECK);
    else
        str = sText_TenDashes;
...
u8 PrintCryScreenSpeciesName(u8 windowId, enum NationalDexOrder num, u8 left, u8 top)
{
    ...
    {
    default:
        for (i = 0; GetSpeciesName(species, DO_NAME_CHECK)[i] != EOS && i < POKEMON_NAME_LENGTH; i++)
            str[i] = GetSpeciesName(species, DO_NAME_CHECK)[i];
