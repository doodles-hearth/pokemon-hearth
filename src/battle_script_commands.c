...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    { ITEM_OINTMENT,        {  35,   _,   _,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_TINY_MUSHROOM,   {  25,  10,   _,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_REPEL,           {   8,  30,   _,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_SUPER_OINTMENT,  {   8,  10,  30,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_POKE_DOLL,       {   8,  10,   9,  30,   _,   _,   _,   _,   _,   _, } },
    { ITEM_BIG_MUSHROOM,    {   3,  10,   9,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_SUPER_REPEL,     {   3,  10,   9,   9,  30,   _,   _,   _,   _,   _, } },
    { ITEM_SACRED_OINTMENT, {   3,   3,   9,   8,   9,  30,   _,   _,   _,   _, } },
    { ITEM_REVIVAL_POWDER,  {   3,   3,   3,   8,   8,   9,  30,   _,   _,   _, } },
    { ITEM_HYPER_OINTMENT,  {   3,   3,   3,   4,   8,   9,   8,  30,   _,   _, } },
    ...
    { ITEM_MAX_OINTMENT,    {   _,   _,   3,   4,   4,   4,   8,   8,   9,  30, } },
    ...
    { ITEM_REVIVAL_HERB,    {   _,   _,   _,   4,   4,   4,   4,   7,   9,   9, } },
...
{
    enum BattlerId effectBattler = se->effectBattler;
    enum BattlerId battlerAtk = cv->battlerAtk;

    const u8 *battleScript = se->script;
    enum MoveEffect moveEffect = se->moveEffect;
    ...
    if (!se->primary && !affectsUser && IsMoveEffectBlockedByTarget(cv->abilities[effectBattler]))
    ...
    case MOVE_EFFECT_TOXIC:
    case MOVE_EFFECT_FROSTBITE:
        if (IsSafeguardProtected(battlerAtk, effectBattler, cv->abilities[battlerAtk]) && !se->primary)
        {
            gBattlescriptCurrInstr = battleScript;
        }
    ...
    case MOVE_EFFECT_FLING:
        if (CanFling(battlerAtk, cv->abilities[battlerAtk]) || gBattleStruct->flungItem == FLUNG_ITEM_REMOVED)
        {
            ...
            flingEffect.effectBattler = effectBattler;
    ...
    case MOVE_EFFECT_BREAK_SCREEN:
        if (B_BRICK_BREAK >= GEN_4)
            i = GetBattlerSide(cv->battlerDef); // From Gen 4 onwards, Brick Break can remove screens on the user's side if used on an ally
        else
...
void BS_ItemCureStatus(void)
{
    ...
    if (!HealStatusConditions(&party[gBattleStruct->itemPartyIndex[gBattlerAttacker]], GetItemStatus1Mask(gLastUsedItem), targetBattler))
    {
        ...
        if (GetItemStatus1Mask(gLastUsedItem) & STATUS1_SLEEP)
            ...
            gBattleMons[targetBattler].volatiles.dreamSleep = FALSE;
    }
