...
{
    GIVEN {
        PLAYER(SPECIES_FURRET) { Ability(ABILITY_FRISK); Item(ITEM_ORAN_BERRY); }
        OPPONENT(SPECIES_SENTRET) { Ability(ABILITY_FRISK); Item(ITEM_ORAN_BERRY); }
    } WHEN {
        TURN {}
    } SCENE {
        ABILITY_POPUP(player, ABILITY_FRISK);
        MESSAGE("The opposing Sentret was frisked, revealing its Oran Berry!");
        ABILITY_POPUP(opponent, ABILITY_FRISK);
        MESSAGE("Furret was frisked, revealing its Oran Berry!");
    }
}
...
DOUBLE_BATTLE_TEST("Frisk triggers for player in a Double Battle after switching-in after fainting")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_WOBBUFFET) { HP(1); }
        PLAYER(SPECIES_FURRET) { Ability(ABILITY_FRISK); }
        OPPONENT(SPECIES_WYNAUT) { Item(ITEM_ORAN_BERRY); }
        ...
        MESSAGE("The opposing Wynaut was frisked, revealing its Oran Berry!");
    }
}
...
DOUBLE_BATTLE_TEST("Frisk triggers for opponent in a Double Battle after switching-in after fainting")
{
    ...
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_POUND) != DAMAGE_CATEGORY_STATUS);
        PLAYER(SPECIES_WYNAUT) { Item(ITEM_ORAN_BERRY); }
        ...
        ABILITY_POPUP(target, ABILITY_FRISK);
        MESSAGE("Wynaut was frisked, revealing its Oran Berry!");
    }
}
